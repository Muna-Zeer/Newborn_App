...
class NewbornExaminationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $newbornExamination = $request->input('newbornName');
        if ($hospitalName) {
            ...
            $newbornExamination->nurse_id = $hospital->id;
        }
        $vaccineName = $request->input('vaccine_name');
        if ($vaccineName) {
            $hospital = VaccinationTabel::where('name', $vaccineName)->first();
            if (!$hospital) {
                // return response()->json(['errors' => 'Invalid hospital Name'], 422);
                $hospital = new VaccinationTabel();
                $hospital->name = $vaccineName;
                $hospital->save();
            }
            $newbornExamination->vaccine_id = $hospital->id;
        }
        //get nurse Name
        ...
        if ($nurseName) {
            ...
            $newbornExamination->midwife_id = $nurse->id;
        }
        //get doctor Name
        ...
        if ($doctorName) {
            ...
            $newbornExamination->doctor_id = $doctor->id;
        }
        // Save the changes
