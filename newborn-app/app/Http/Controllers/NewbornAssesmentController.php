...
class NewbornAssesmentController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        // If the doctor_id, midwife_id, or nurse_id fields are present, ensure that the corresponding model exist
        if (isset($validatedData['doctor_id'])) {
            Doctor::FindOrFail($validatedData['doctor_id']);
        }
        if (isset($validatedData['midwife_id'])) {
            Midwife::FindOrFail($validatedData['midwife_id']);
        }
        if (isset($validatedData['nurse_id'])) {
            Nurse::FindOrFail($validatedData['nurse_id']);
        }
        // Create a new NewbornAssessment record
        $newbornAssessment = Newborn_Assessment::create($validatedData);
        $doctor=isset($validatedData['doctor_id'])?Doctor::findOrFail($validatedData['doctor_id']):null;
        $midwife=isset($validatedData['midwife_id'])?Midwife::findOrFail($validatedData['midwife_id']):null;
        $nurse=isset($validatedData['nurse_id'])?Nurse::findOrFail($validatedData['nurse_id']):null;




        // If doctor_id, midwife_id, or nurse_id are present, associate the corresponding model with the new NewbornAssessment record
        if ($doctor) {
            $newbornAssessment->doctor()->associate($doctor);
        }
        if ($midwife) {
            $newbornAssessment->midwife()->associate($midwife);
        }
        if ($nurse) {
            $newbornAssessment->nurse()->associate($nurse);
        }

        // Save the associations
        $newbornAssessment->save();

        // Return a response indicating success
        return response()->json(['message' => 'Newborn assessment created successfully.'], 201);
    }
